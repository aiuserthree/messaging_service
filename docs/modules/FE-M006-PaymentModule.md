...
- **분배 관리 페이지**:
  - "현재 디파짓 잔액" → "현재 잔여 포인트"로 변경
  - "발신번호별 금액 분배" → "발신번호별 포인트 분배"로 변경
  - "디파짓 잔액을 발신번호별로 분배하세요" → "잔여 포인트를 발신번호별로 분배하세요"로 변경
  - "현재 잔액" → "현재 잔여 포인트"로 변경
  - "분배 금액" → "분배 포인트"로 변경
  - "디파짓 잔액" → "잔여 포인트"로 변경
  - "분배 금액 합계" → "분배 포인트 합계"로 변경
  - "분배 후 잔액" → "분배 후 잔여 포인트"로 변경
